...
import com.example.happypet.model.ResponseModel
...
class HomeViewModel @Inject constructor(private val retrofitRepository: RetrofitRepository) :
    ...
    val _characters = MutableLiveData<List<Character>>()
    var params by mutableStateOf(null)

    val characters: LiveData<List<Character>>
        get() = _characters
    private val _characterFlow by lazy { MutableStateFlow(ResponseModel.Success(emptyList())) }
    ...
    fun getCharacters(page: Int) {
        viewModelScope.launch {
            retrofitRepository.getCharacters(page).let {
                ...
                _characterFlow.value = ResponseModel.Success(characters)
